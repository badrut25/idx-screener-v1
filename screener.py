# @title OOP Complete Suite: 5 Screeners (termasuk MFI+Stoch) + Telegram + WIB
import numpy as np
import pandas as pd
...
class Config:
    ...
    # KONFIGURASI MFI & STOCHASTIC BARU
    MFI_LEN = 14
    MFI_OS = 20
    MFI_OB = 80
    STOCH_K = 14
...
class TechnicalIndicators:
    ...
    # === FITUR BARU: MFI & STOCHASTIC ===
    @staticmethod
    def add_mfi(df, length=14, os=20, ob=80):
        tp = (df["High"] + df["Low"] + df["Close"]) / 3.0
        rmf = tp * df["Volume"]
        pos_flow = np.where(tp > tp.shift(1), rmf, 0.0)
        neg_flow = np.where(tp < tp.shift(1), rmf, 0.0)
        
        pos_flow_sma = pd.Series(pos_flow).rolling(window=length).mean()
        neg_flow_sma = pd.Series(neg_flow).rolling(window=length).mean()
        
        money_ratio = np.where(neg_flow_sma == 0, 100.0, pos_flow_sma / neg_flow_sma)
        df["MFI"] = np.where(neg_flow_sma == 0, 100.0, 100.0 - (100.0 / (1.0 + money_ratio)))
        
        # Sinyal crossunder batas oversold (Titik Hijau) dan crossover overbought
        df["MFI_Oversold"] = (df["MFI"].shift(1) > os) & (df["MFI"] < os)
        df["MFI_Overbought"] = (df["MFI"].shift(1) < ob) & (df["MFI"] > ob)
        return df

    @staticmethod
    def add_stoch(df, periodK=14, smoothK=3, periodD=3):
        lowest_low = df["Low"].rolling(window=periodK).min()
        highest_high = df["High"].rolling(window=periodK).max()
        
        stoch_raw = 100 * (df["Close"] - lowest_low) / (highest_high - lowest_low)
        df["Stoch_K"] = stoch_raw.rolling(window=smoothK).mean()
        df["Stoch_D"] = df["Stoch_K"].rolling(window=periodD).mean()
        
        # Sinyal crossover (Golden Cross)
        df["Stoch_GC"] = (df["Stoch_K"] > df["Stoch_D"]) & (df["Stoch_K"].shift(1) <= df["Stoch_D"].shift(1))
        return df
    
    @staticmethod
    def add_mfi_stoch_logic(df):
        mfi_menunggu = False
        buy_signal = np.zeros(len(df), dtype=bool)
        
        mfi_os_vals = df["MFI_Oversold"].values
        mfi_ob_vals = df["MFI_Overbought"].values
        stoch_gc_vals = df["Stoch_GC"].values
        
        for i in range(len(df)):
            if mfi_os_vals[i]:
                mfi_menunggu = True
            
            # Jika MFI kembali ke overbought sebelum Stoch GC, batalkan status menunggu (safety filter)
            if mfi_ob_vals[i]:
                mfi_menunggu = False
                
            trigger = stoch_gc_vals[i] and mfi_menunggu and not mfi_os_vals[i]
            buy_signal[i] = trigger
            
            if trigger:
                mfi_menunggu = False # Reset memory setalah trigger sukses
                
        df["MFI_Stoch_Buy"] = buy_signal
        return df
...
class ScreenerEngine(SignalLogic):
    ...
    def _prepare_df(self, df):
        ...
        # Load MFI + Stoch 
        df = TechnicalIndicators.add_mfi(df, Config.MFI_LEN, Config.MFI_OS, Config.MFI_OB)
        df = TechnicalIndicators.add_stoch(df, Config.STOCH_K, Config.STOCH_SMOOTH_K, Config.STOCH_D)
        df = TechnicalIndicators.add_mfi_stoch_logic(df)
    ...
    # === NEW: MFI + STOCHASTIC GC SCREENER ===
    def run_mfi_stoch_screener(self):
        results = []
        for t, df in self.data_dict.items():
            if (df := self._prepare_df(df)) is None: continue
            for j in range(max(1, len(df) - Config.RECENT_BARS), len(df)):
                if bool(df["MFI_Stoch_Buy"].iloc[j]) and (bool(df["Volume_OK"].iloc[j]) and bool(df["Volume_Confirm"].iloc[j])):
                    res = self._package_result(t, df, j)
                    res["Pattern_Label"] = "🟢 MFI + Stoch GC"
        ...
        for t, df in self.data_dict.items():
            ...
            for j in range(max(1, len(df) - Config.RECENT_BARS), len(df)):
                ...
                if bool(df['Kernel_Red_to_Green'].iloc[j]) and c_vol:
                    ...
                    res = self._package_result(t, df, j)
                    res["Pattern_Label"] = f"🟢 Bull (ML: {ml_score})" 
                    res_bull.append(res); break 
                
                elif bool(df['Kernel_Green_to_Red'].iloc[j]) and c_vol:
                    ml_score = self.is_lorentzian_bullish(df, j, neighbors=8)
                    res = self._package_result(t, df, j)
                    res["Pattern_Label"] = f"🔴 Bear (ML: {ml_score})" 
                    res_bear.append(res); break
...
def send_telegram_message(data_dict, date_str):
    ...
    msg += f"<b>🤖 ML Lorentzian (R-to-G):</b>\n{get_tickers(data_dict['lorentzian_bull'])}\n\n"
    msg += f"<b>🟢 MFI + Stoch GC:</b>\n{get_tickers(data_dict['mfi_stoch'])}\n\n"
...
if __name__ == "__main__":
    ...
    # EKSEKUSI SCREENER BARU
    res_mfi_stoch = add_fundamentals(screener.run_mfi_stoch_screener())
    ...
    export_data = {
        ...
        "mfi_stoch": res_mfi_stoch.to_dict(orient="records") if not res_mfi_stoch.empty else [],
